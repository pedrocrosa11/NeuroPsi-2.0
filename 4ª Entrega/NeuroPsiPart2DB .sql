...
create table Test (testId int auto_increment primary key, testState enum("Pending", "Completed", "Filed", "Reschedule", "Canceled") default "Pending", comment varchar(300), assignedDate date, completedDate date, test_attribId int, test_routeId int, foreign key (test_attribId) references Attribution (attribId), foreign key (test_routeId) references Route (routeId));
...
create table Discalculia (discalcId int auto_increment primary key, firstNumber int, sign char, secondNumber int, result varchar(10), correctAnswer int, discalc_testId int, foreign key (discalc_testId) references Test(testId));
